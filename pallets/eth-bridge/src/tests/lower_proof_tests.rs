...
mod lower_proofs {
    ...
    fn multiple_mixed_requests_with_same_id_can_be_processed() {
        ...
        ext.execute_with(|| {
            ...
            context.expected_lower_msg_hash = "f6567b5fc754d7b5ec6543e28c68373851ec1cd91a7c228c8f1e4c40f8d9fd8d".to_string();
            call_ocw_and_dispatch(&context, offchain_state.clone(), &pool_state, 1u64, 2u64);

            // Because this is a send, active request doesn't change (next phase is to send and corroborate)
            assert_eq!(true, ActiveRequest::<TestRuntime>::get().unwrap().request.id_matches(&tx_id));
            ...
            context.expected_lower_msg_hash = "c7b00196754f72fbdd51b612d0eb8c69495e8d7e092cd26d46d780d0ea15b4a8".to_string();

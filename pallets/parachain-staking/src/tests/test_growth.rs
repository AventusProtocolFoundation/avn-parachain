...
mod growth_info_recorded_correctly {
    ...
    //             assert_eq!(growth.total_points, DEFAULT_POINTS * ErasPerGrowthPeriod::get() * 2);
    // // 2 collators

    //             // Check that total stake matches eras 1 and 2 because payouts are delayed by 2
    // eras             assert_eq!(
    //                 growth.total_stake_accumulated,
    //                 raw_era_data.get(&1).unwrap().stake + raw_era_data.get(&2).unwrap().stake
    //             );

    //             // Because `Reward` is not backdated, get the sum of reward for the current eras
    // we             // snapshoted this GrowthPeriod
    //             assert_eq!(
    //                 growth.total_staker_reward,
    ...
    //             // current era starts at the actual era this growth period has been created (3)
    // and             // ends at the last era accumulated by this growth period (4)
    ...
    //                 // This is a bit tricky because `Reward` is not backdated. We pay whatever
    // was                 // in the reward pot at the time of payout therefore it is the
    //                 // sum of rewards for the non backdated eras that make up this GrowthPeriod)
    //                 assert_eq!(
    //                     growth.total_staker_reward,

...
//             (nominator(), 10000),
//         ])
//         .with_candidates(vec![(collator_1, COLLATOR1_OWN_STAKE), (collator_2,
// COLLATOR2_OWN_STAKE)])         .with_nominations(vec![(nominator(), collator_1,
// NOMINATOR4_STAKE)])         .build()
//         .execute_with(|| {
...
//             // Show a list of events we expect when rolling to era 2. Note the absence of
// rewards.             let expected_events = vec![
...
//             // Show that reward pot balance and locked era balance are 0 because everything has
// been             // paid out for all collators
//             assert_eq!(Balances::free_balance(&reward_pot_account_id), 0);
//             assert_eq!(ParachainStaking::locked_era_payout(), 0);

//             // check that distributing rewards clears awarded points
//             assert!(
//                 ParachainStaking::awarded_pts(ERA_BLOCKS_HAVE_BEEN_AUTHORED,
// collator_1).is_zero()             );
//         });
// }
...
mod compute_total_reward_to_pay {
    ...
    mod fails_when {
        ...
        //     ExtBuilder::default()
        //         .with_balances(vec![(collator_1, 10000), (collator_2, 10000), (tx_sender(),
        // 10000)])         .with_candidates(vec![
        //             (collator_1, COLLATOR1_OWN_STAKE),
        //             (collator_2, COLLATOR2_OWN_STAKE),
        //         ])
        //         .build()
        ...
        //             Balances::make_free_balance_be(&reward_pot_account_id,
        // bad_reward_pot_balance);

        //             // attempt to compute total reward to pay
        //             let new_total_reward_to_pay =
        // ParachainStaking::compute_total_reward_to_pay();
...
mod reward_payout_fails_when {
    ...
    //             assert_eq!(Balances::free_balance(&reward_pot_account_id),
    // bad_reward_pot_balance);             // locked era balance did not change
    //             assert_eq!(ParachainStaking::locked_era_payout(), collator2_reward);
    //         });
    // }
}

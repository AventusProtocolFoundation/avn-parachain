...
pub struct GrowthOffence<Offender> {
    ...
    pub offence_type: GrowthOffenceType,
}

impl<Offender: Clone> Offence<Offender> for GrowthOffence<Offender> {
    const ID: Kind = *b"growths::offence";
...
pub fn create_and_report_growth_offence<T: crate::Config>(
    ...
    if offenders.len() > 0 {
        let invalid_event_offence = GrowthOffence {
        ...
        if !T::ReportGrowthOffence::is_known_offence(
            ...
            <crate::Pallet<T>>::deposit_event(Event::<T>::GrowthOffenceReported {
                offence_type,
                offenders,
            });
        }

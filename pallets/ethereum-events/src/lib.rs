...
pub mod pallet {
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn offchain_worker(block_number: BlockNumberFor<T>) {
            ...
            let is_primary_eth_validator =
                AVN::<T>::is_primary_eth_validator(&this_validator.account_id);
            if is_primary_eth_validator.is_err() {
                log::error!("Error checking if validator can check result");
                return
            }

            // =============================== Main Logic ===========================
            if is_primary_eth_validator.expect("Already checked for error.") {
    ...
    impl<T: Config> ValidateUnsigned for Pallet<T> {
        ...
        fn validate_unsigned(_source: TransactionSource, call: &Self::Call) -> TransactionValidity {
            ...
            } = call
            {
                ...
                if AVN::<T>::is_primary_eth_validator(&result.checked_by)

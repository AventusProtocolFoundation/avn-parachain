...
fn test_get_event_to_validate_all_checked_by_us() {
    with_offchain_worker(eth_events_test_with_validators()).execute_with(|| {
        ...
        assert!(EthereumEvents::get_next_event_to_validate(&this_validator_account_id, 0u64.into()).is_none());
    });
}
...
fn test_get_event_to_validate_past_challenge_window() {
    with_offchain_worker(eth_events_test_with_validators()).execute_with(|| {
        ...
        assert!(EthereumEvents::get_next_event_to_validate(&new_validator_account_id, 0u64.into()).is_none());
    });
}
...
fn test_get_event_to_validate_1_good_event() {
    with_offchain_worker(eth_events_test_with_validators()).execute_with(|| {
        ...
        assert!(EthereumEvents::get_next_event_to_validate(&new_validator_account_id, 0u64.into()).is_some());
    });
}
...
fn test_get_event_to_validate_first_good_event() {
    with_offchain_worker(eth_events_test_with_validators()).execute_with(|| {
        ...
        let (next_event_to_validate, counter, _) =
            EthereumEvents::get_next_event_to_validate(&this_validator_account_id, 0u64.into()).unwrap();
...
fn test_get_event_to_validate_mixed() {
    with_offchain_worker(eth_events_test_with_validators()).execute_with(|| {
        ...
        let (next_event_to_validate, counter, _) =
            EthereumEvents::get_next_event_to_validate(&new_validator_account_id, 0u64.into()).unwrap();
        let expected_event_id = EthEventId {
            signature: ValidEvents::AddedValidator.signature(),

...
frame_support::construct_runtime!(
    pub enum TestRuntime
    {
        ...
        AvnConsensus: pallet_avn_consensus::{Pallet, Call, Storage, Event<T>},
    }
);
...
thread_local! {
    pub static CONSENSUS_LOG: RefCell<Vec<(u32, Vec<u8>, u32)>> = RefCell::new(Vec::new());
}

/// Router that receives finalized consensus payloads.
pub struct TestConsensusRouter;
impl pallet_avn_consensus::pallet::OnConsensusReached<TestRuntime> for TestConsensusRouter {
    fn on_consensus(
        feed_id: u32,
        payload: Vec<u8>,
        round_id: u32,
    ) -> frame_support::dispatch::DispatchResult {
        CONSENSUS_LOG.with(|l| l.borrow_mut().push((feed_id, payload, round_id)));
        Ok(())
    }
}
...
// OCW plumbing: required by bounds
...
// ----------------- Validators helpers (same as your file) -----------------
...
pub fn create_validator(author_id: u64) -> Validator<UintAuthorityId, AccountId> {
    Validator {
        ...
        account_id: TestAccount::new([author_id as u8; 32]).account_id(),
    }
}
...
// small helper
pub fn clear_router_log() {
    CONSENSUS_LOG.with(|l| l.borrow_mut().clear());
}

pub fn router_log_len() -> usize {
    CONSENSUS_LOG.with(|l| l.borrow().len())
}

pub fn router_log_get(i: usize) -> (u32, Vec<u8>, u32) {
    CONSENSUS_LOG.with(|l| l.borrow()[i].clone())
}

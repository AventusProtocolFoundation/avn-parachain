use sp_avn_common::event_types::{AddedValidatorData, AvtGrowthLiftedData, Error, EthEvent, EthEventId, EventData, LiftedData, NftCancelListingData, NftEndBatchListingData, NftMintData, NftTransferToData, ValidEvents};
...
fn signature_to_event_type(signature: sp_core::H256) -> Option<ValidEvents> {
    match signature {
        signature if signature == ValidEvents::AddedValidator.signature() => Some(ValidEvents::AddedValidator),
        ...
        signature if signature == ValidEvents::NftTransferTo.signature() => Some(ValidEvents::NftTransferTo),
        signature if signature == ValidEvents::NftCancelListing.signature() => Some(ValidEvents::NftCancelListing),
        signature if signature == ValidEvents::NftEndBatchListing.signature() => Some(ValidEvents::NftEndBatchListing),
        signature if signature == ValidEvents::AvtGrowthLifted.signature() => Some(ValidEvents::AvtGrowthLifted),
        _ => None,
    }
}

fn parse_event_data(event_type: ValidEvents, data: Vec<u8>, topics: Vec<Vec<u8>>) -> Result<EventData, Error> {
    match event_type {
